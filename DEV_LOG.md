...
## Feature 66: low-quality flashcard fix (`MIN_BACK_CHARS`) plus a real heading-detection blind spot (Google-Docs-exported PDFs) — both found and fixed against all 25 real `TestDoc/` files
...
**Fixed in the same session, once the user asked to proceed**: `classifyLine` gets a third fallback signal alongside size-ratio and bold-name matching — `RawLine` now tracks per-line character counts by raw pdf.js font resource name (`fontChars`), and the document's dominant font resource is computed the same char-weighted-vote way `bodySize`/`bodyX` already are. A line at body-like size, with no real list marker, where a large majority of its characters (`>= BOLD_MAJORITY_RATIO`, the same 0.6 threshold the real-bold-lead-in case already uses) come from a *different* font resource than the document's dominant one, is now treated as a subheading candidate. This directly targets the Google-Docs blind spot above: real headings in that export path are visually distinct (a different underlying font resource, `g_d0_f3` vs body's `g_d0_f5`) even though pdf.js can't expose that difference as "bold" or a size jump.

**Two real false positives found by testing the fix itself, not assumed** (same discipline as Feature 65): a lone bullet glyph with no attached text (`"•"`, orphaned in `TestDoc/"7ASD810S (1).pdf"`) was being promoted to a heading purely for using a different, glyph-only font — fixed with a cheap, unconditionally-safe guard requiring at least one real letter (`/\p{L}/u`) before *any* line can be classified heading/subheading. A denser, real false positive in `TestDoc/"4-Week5D PRS821 Network Attacks-1of2.pdf"`: that document's own list has a pre-existing reading-order scramble (a separate, deeper bug, not touched here) that occasionally merges a single bold-styled lead-in term (a list item's own emphasized name, e.g. "Stuxnet") with surrounding body-font fragments into one "heading"-classified line. Tried tightening the font-majority ratio from `0.6` to `0.95` to demand a line be almost entirely non-body-font before trusting it — this didn't fix the 4-Week5D case (its offending line's font mix apparently already clears even a 0.95 bar) and broke a different, previously-clean document (`TestDoc/"2-Week5B PRS821 Database Security-S1.pdf"` dropped from 9 correctly-detected headings to 2). Reverted to `0.6`, keeping only the letter-count guard. **Known, accepted, documented remaining cost**: one garbled heading persists in 4-Week5D (11 of its 12 flashcards are clean); chasing it further needs a genuinely different signal (its root cause is that document's reading-order bug, not the font heuristic itself) rather than more threshold-tuning on this one.

### How it was validated

`npx tsc --noEmit` and `eslint` clean on every iteration. Real Supabase-backed verification, run three full times (once per real code change, matching Feature 65's own precedent): all 25 real `TestDoc/` files uploaded through the real running app under a fresh throwaway account each round, real `extracted_text` pulled via service-role query, `generateFlashcards` run directly against each document's actual stored text, every card's back length and heading count compared round-to-round. **Net result across the full corpus**: 9 of the 10 previously-zero-card documents now produce real, substantive flashcards (`Network Security`, `Application Security`, `Preventing Password Attacks`, `Stolen Database Backups`, and similar genuine section headings, not garbage); the 10th (`TestDoc/"ID 2026-03-11.pdf"`, 590 chars) still correctly produces none — it has no real heading structure to find, which is honest, not a bug. Zero thin-backed (`< 60` char) cards anywhere. One known, narrow, documented false positive remains in one document (above).

---

## Feature 67: swecom.pdf's References section was genuinely scrambled — three separate real bugs found and fixed via a live headless-browser batch run against all 40 real `TestDoc/` files, plus numbered-list and blockquote fidelity added

The user provided two new screenshots (`SystemImage/"no structre 1.png"`, `"no strutre 2.png"`) showing extracted text reading as flat, undifferentiated paragraphs with no real structure, and asked for repeated real-document testing across layout, headings, bullets, numbered lists, and quotes. Built a throwaway Playwright + real-vite-dev-server harness (a temporary `src/routes/dev-batch-extract.tsx` route, removed before finishing — never committed) that runs the actual `extractPdfText`/`extractDocxStructuredText` code against every real file in `TestDoc/` (40 files: the folder has grown again) and dumps structural stats plus full text per file, matching this project's established real-corpus-testing precedent (Features 65/66).

**Screenshot 1 (swecom.pdf citations) traced to three independent, compounding bugs, not one:**

1. **`classifyLine` misreads a citation's own wrapped continuation line as a heading.** `looksLikeContinuationLine`'s lowercase-start heuristic (Feature 65) was built for a *markerless* list, where capitalization is the only signal available — but a citation that already opened with an explicit, unambiguous marker (`[Babich 1986]`) has no such ambiguity, and its wrapped continuation is very often a capitalized book/standard title ("Management: Coordination for Team Productivity, Addison-"), which the case-based heuristic wrongly read as a new item — and, since it also happened to clear `classifyLine`'s font-size/bold heading thresholds, as a heading outright. Fixed by tracking whether the *currently open* bullet was started by a real marker (`bulletBuffer.startedWithMarker`); when it was, nothing but a new marker or a genuinely complete (sentence-terminal) buffer can end it, regardless of what case the continuation starts with — checked from all three of `formatStructuredText`'s heading/subheading/bullet branches via one shared `bulletBufferShouldAbsorb` helper.
2. **`findColumnGutter`'s two-column-page detector false-positives on a hanging-indent list.** A citation's marker sits at the body left margin, its continuation a short, fixed distance to the right — exactly the two-cluster shape a real column split produces. The gap/band checks were computed against only *these lines'* own observed min/max x, not the real page — for a list that never uses more than a sliver of the page's actual width, that made the observed span artificially tiny, so an 18-unit hanging indent (on a 504-unit-wide real page, confirmed via a direct `pdfjs-dist` page-geometry dump) cleared both checks trivially, splitting the whole references section into "all citation keys" then "all continuations," reordered independently by y. Fixed by anchoring both checks to the real page width/left edge (`page.view`, threaded through from the caller) instead of the lines' own span — a real two-column document (`TestDoc/"Git Cheat Sheet (2-column test).pdf"`, the case this heuristic exists for) still spans a real fraction of the actual page and is unaffected.
3. **The running-header/footer noise filter over-triggers on a legitimately repeated bibliography.** swecom.pdf's core citations are genuinely, verbatim cited from multiple, widely-separated chapter reference lists (the same document already established "References" itself repeats this way, Feature "noise" comment). The old rule required subheadings to cluster within a narrow page span but let "body"/"bullet" kinds through unconditionally on any 3+ page repeat — a repeated citation looks identical to a real running header under a page-count-only test, since both repeat 3+ times, scattered. The real, reliable difference a page-index check can't see: a template-placed header/footer sits at the *exact same y position* on every page it appears on, while a repeated citation lands wherever that page's own paragraph flow happens to put it. Replaced the kind-based branching entirely with a direct y-position-consistency check (`NOISE_Y_TOLERANCE = 6`, all occurrences of a candidate string must share that tight a y-spread) — a real repeated citation's scattered y now correctly survives, while a real running header/footer (which the y-check catches regardless of which `LineKind` it happens to classify as, closing a second, separate gap the old kind-based rule also had) is still removed.

Fixing all three (not just the first) was necessary — the first fix alone still left visibly scrambled/incomplete citations, because the underlying feed of lines into `formatStructuredText` was still wrong (reordered by bug 2) or silently missing lines (deleted by bug 3) before the buffering logic ever ran.

**Screenshot 2 (`UPDATED_NAM_CLEARING_INSTRUCTION.docx`, a Namibian customs form) investigated, not a code bug**: its letterhead lines (department name, tel, address) are genuinely separate *floating Word text boxes* in the source file (confirmed via the raw OOXML — `mc:AlternateContent`/`wps:txbx`), not paragraphs or a table. mammoth flattens each into its own bare `<p>`, with no positional/grouping context surviving the conversion — a real, hard limitation (no coordinate data comes through the way pdf.js's `item.transform` gives the PDF path), not a bug in `docx-extract.ts`. The one non-text element mammoth produced (a single `<img>`, silently dropped by the existing fallback branch) was checked directly — it decodes to a small "Namibia Revenue Agency" wordmark, not missing form data, so nothing substantive is actually lost. Documented as a known gap rather than attempting a full custom OOXML shape-position parser; the form's actual field tables (country/CPC checkboxes, the eight numbered instruction sections) already extract correctly.

**Two more structural gaps the user explicitly named, fixed while already in this code:**

- **Numbered/lettered list items were losing their own numbering**, collapsing into plain bullets indistinguishable from an unordered list (`stripBulletPrefix` stripped a numbered marker exactly like a bullet glyph). Added `stripGlyphBulletPrefix` (pdf-extract.ts) so a numbered/lettered marker stays in the item's own text — verified against a real lettered multiple-choice quiz (`TestDoc/"Mastery Learning Quiz 2..."pdf`, options `a. b. c. d.`), which now preserves each option's letter. `docx-extract.ts`'s `<ol>` handling (Word doesn't store list numbers as visible text at all, only via numPr metadata) now reconstructs a sequential number per item. `StructuredText.tsx` recognizes a numbered/lettered-marked bullet run and renders a real `<ol>` (auto-numbered decimally) instead of a plain disc bullet — a real visual distinction between "bullet points" and "number points," not just matching raw text shape.
- **Word's Quote/Intense Quote paragraph styles rendered as indistinguishable plain prose** (`docx-extract.ts`'s own prior comment already flagged this honestly as unhandled). Mapped both styles to a real `<blockquote>` in mammoth's styleMap (same mechanism already proven for Title/Subtitle → h1/h2) and added a `> `-prefixed markup convention, rendered by `StructuredText.tsx` as a real styled blockquote.

### How it was validated

`npx tsc --noEmit`, `prettier --write`, and a fresh full 40-file batch re-run (via the real headless-browser harness above, never a mock) after *each* of the five changes — not just once at the end. Confirmed zero extraction failures at every step. The three References-section fixes were each individually confirmed against a direct raw `pdfjs-dist` page dump (not just the app's own output) before being trusted: page 28's true item order, page 504-unit width, and the exact y-values of the "Babich"/"IEEE 828-2012" citations were all read directly from pdf.js, not inferred. Post-fix, both real occurrences of the Babich and IEEE-828-2012 citations (pages 19 and 124, a shared bibliography entry) read as complete, correctly-ordered single items with no scrambling, and the original screenshot's specific misread heading ("## Configuration Management in Systems and Software") no longer appears anywhere in the document. The noise-filter change is a net restoration of previously-deleted real content across many documents in the corpus, not just swecom.pdf — several other files (the `ASD810S` lecture-slide series in particular) had 0 detected subheadings before this fix and correctly detect real ones now (their own per-slide "Introduction" title, reprinted per slide, still appears — a judged acceptable trade-off: real, if repetitive, content surviving beats a real citation silently vanishing).

---

## Feature 68: continued real-corpus testing surfaced a second citation-list-shaped bug in a different document (`PG-Slides-RIT-2023- Mr Peter.pdf`) — a bare-digit numbered list, and a real bug in `formatStructuredText`'s own heading/bullet interaction that Feature 67's fixes exposed but didn't cause

The user asked to keep testing at volume and kept improving after Feature 67 shipped. Restored the same real headless-browser batch harness (removed again before finishing) and re-ran it against all 40 `TestDoc/` files, then spot-checked the documents whose structural counts moved the most since the last run.

**`PG-Slides-RIT-2023- Mr Peter.pdf` (a PowerPoint export) had its own hanging-indent-list bug, one step removed from Feature 67's citation case**: this deck's numbered-list glyphs render as a bare digit with no trailing punctuation at all ("2 Tricky installation...", not "2. Tricky installation..."). `NUMBERED_PATTERN` requires a `.`/`)` after the digit, so these went completely unrecognized as list markers — worse than Feature 67's citation case, since here `classifyLine` didn't even classify the items as "bullet" via `hasListMarker`, and separately, `mergeAdjacentTableCells` paired a numbered item with its own wrapped continuation into a fake two-cell table (the exact shape Feature 67's citation fix already guards against, just not for this unpunctuated variant). Two entirely separate numbered lists on the same slide read as one garbled run-on paragraph, with a fake `| |` table embedded partway through. Fixed with a new `BARE_NUMBER_MARKER_PATTERN` (a digit followed by a capitalized word — the same "does the wrapped continuation start with a capital" signal `looksLikeContinuationLine` already uses elsewhere, applied here to rule out an ordinary sentence that happens to start with a bare number), folded into both `hasExplicitBulletMarker` and `classifyLine`'s `hasListMarker`.

**Fixing that surfaced a third, genuinely new bug in `formatStructuredText` itself, found by re-testing the fix, not assumed**: once "Steep learning curve:" (a real subheading between the two numbered lists) correctly stopped getting absorbed into the *previous* list's last item, it still rendered in the *wrong position* — appearing before that item instead of after it, even though the items were processed in the correct order the whole time. Root cause: the heading/subheading case's own `bulletBufferShouldAbsorb` check, when it correctly decides *not* to absorb, has never called `flushBullet()` — a gap present in the very first version of this function, just never triggered before, since nothing previously both opened a marker-started bullet *and* correctly rejected a later heading from absorbing it. A dangling, not-yet-terminal bullet buffer stayed open across the heading, only getting flushed once some *later* bullet-classified line called `flushBullet()` at the top of its own case — pushing the earlier, stale bullet to `blocks` *after* the heading that was supposed to follow it. Fixed by calling `flushBullet()` in the heading/subheading case whenever the absorb-check rejects, mirroring what the `bullet` case already does before opening a new item. Also added indentation as a second signal alongside "started with an explicit marker": a genuine continuation of a list item is never *less* indented than that item's own opening marker (`bulletBuffer.markerX`, tracked per-buffer now) — a real subheading returning to the document's base margin is always more flush-left than the list it's interrupting, which is what let this specific case (item ending with no trailing punctuation, so "was it complete?" alone couldn't tell) resolve correctly.

### How it was validated

`npx tsc --noEmit`, `prettier --write`, and a full 40-file batch re-run after each of the three changes (bare-number pattern, the heading/bullet ordering fix, and the x-position check together) — zero extraction failures throughout. `PG-Slides-RIT-2023- Mr Peter.pdf` was confirmed directly against a raw `pdfjs-dist` item dump of the exact affected page (all fragment x/y positions read straight from pdf.js, not inferred) before and after each fix, and now reads as two clean, correctly-ordered, correctly-separated numbered lists with a real subheading between them, no fake table. The corpus-wide re-run showed broad, real restoration elsewhere too — `TestDoc/"1-Week5A PRS821 Domains of Security-S1.pdf"` (the exact Google-Docs-export document Feature 66 targeted) went from 5 to 21 correctly-detected subheadings; spot-checked and all 16 newly-surfaced ones are real section titles ("Network Security", "Cloud Security", "Disaster Recovery," and similar), previously swallowed into a preceding list item by the same dangling-bulletBuffer bug this fixes, not new false positives.

---

## Feature 69: a real (partial, honestly-scoped) fix for multi-column/infographic PDF gluing, found via a broader qualitative pass over the rest of the `TestDoc/` corpus, plus researched and rejected an external-dependency approach in favor of building on what's already in place

The user asked to keep testing broadly and to research whether an external open-source component could help further. Spot-checked the remaining not-yet-individually-reviewed files in `TestDoc/`: plain, linear-flowing documents (a full thesis proposal, an official course outline, another thesis-style `.docx`) extract cleanly — real headings, real tables, real bullets, correct paragraphs, no issues found. Two PowerPoint-style infographic exports (`TestDoc/"Problem Statement.pdf"`, `TestDoc/"Literature Review- March 2023.pdf"`) showed a different, previously undiscovered real bug: text from separate, unrelated visual elements (cycle-diagram labels, side-by-side callout-box titles) reading as one glued blob, e.g. `"Formulate aPublish QuestionFindings"` — four separate diagram labels concatenated with no separation at all.

**Root cause, and why it's only partially fixable right now**: the line-clustering step (`extractPdfTextViaPdfJs`'s fragment loop) already tracks a "cell boundary" whenever two same-y fragments have a wide horizontal gap (`CELL_GAP_RATIO`) — but a line carrying multiple such cells that never goes on to satisfy `detectTableRows`' own `MIN_TABLE_ROWS` requirement (a single isolated row, no repeating structure below it to confirm a real table) still had those cells flattened back into one continuous string with nothing but an ordinary word-sized space at each boundary — reading as a run-on sentence instead of what it visually is. Fixed with a new `reconstructOrphanedCellText`: any line with 2+ detected cells that `detectTableRows` declined to confirm as part of a real table now rejoins those cells with a clearly visual `" — "` separator instead. This is a genuine, if partial, improvement — it only helps where the fragment-clustering pass already recognized a wide-enough gap to register as a separate cell in the first place. A second, harder case in the *same* documents (two diagram labels sitting close enough together — a compact circular/radial diagram, not a wide multi-column layout — that they never register as separate cells at the fragment level at all, e.g. "Reference PointClearly Define Solution" gluing two labels with literally zero gap) remains open and is honestly documented as such, not papered over; a real fix needs the fragment-clustering step itself to also weigh horizontal distance when deciding whether two same-y fragments belong on one line, which is a larger, riskier change to a piece of logic every single document's extraction depends on — not attempted this session.

**Researched external tooling before writing any new code, per this project's own established practice (see Features 44/62's `pdfexcavator`/`LiteDoc` evaluations)**: the prominent 2026 open-source document-structure parsers (Docling, MinerU, Marker, olmOCR) are Python/ML-based, server-side tools — architecturally incompatible with this app's offline-first, everything-in-the-browser design, the same reason `pdfexcavator` was rejected in Feature 44. For the *other* known gap (Word's floating text boxes losing positional context, from Feature 67's customs-form finding), confirmed via a direct raw OOXML read that real position data already exists in every `.docx`'s own XML (`<wp:positionH relativeFrom="page"><wp:posOffset>802848</wp:posOffset>`) — the same class of signal pdf.js's `item.transform` already gives the PDF path. This is buildable as a small custom module reading `document.xml` directly (JSZip + `DOMParser`, both already in use via mammoth's own dependency chain and this file's existing code) — no new external dependency needed, and lower-risk than adopting an unproven one. Scoped as a real, if larger, next initiative rather than built this session.

### How it was validated

`npx tsc --noEmit`, `prettier --write`, and a full 40-file batch re-run — zero extraction failures, zero regressions (every real table's own count stayed exactly the same across the whole corpus, confirming the fix genuinely only touches lines `detectTableRows` had already declined). `Problem Statement.pdf` and `Literature Review- March 2023.pdf` were both re-dumped and read directly before and after: wide-gap boundaries that previously glued without any separator now read as clearly distinct items (`"Justify Need for — Focus Project — Reference Point..."`); the honestly-remaining tight-gap case within the same lines was confirmed still present and left as documented, open, future work rather than claimed fixed.

---

## Feature 70: real financial/legal documents (a client risk questionnaire, City of Windhoek utility bills, an affidavit) surfaced a real, fixed row-number-column bug, plus fresh, concrete evidence for the already-scoped column-aware-clustering work

The user provided several new real documents (a financial-advisory risk questionnaire, two City of Windhoek utility bills, a cohabitation-declaration affidavit, an investment offering, an investment mandate) along with reference `.md` files showing what good extraction of the questionnaire should look like — a real 9-column table (row number, question text, then a 7-point rating scale).

**Root-caused and fixed a real, distinct bug in `TestDoc/"Client Risk Questionnaire-Joao.pdf"`**: stray 1-2 digit question numbers were showing up wedged mid-sentence in the extracted text (`"...to put 8 towards this investment"`, `"...restructured under new 9 management"`). A direct raw `pdfjs-dist` fragment dump of the affected page showed why: every question's row number sits in its own narrow left column at `x=50`, while the question text itself starts at `x=64` — a real, deliberate two-column form layout. The two symptoms depend on where that number's own y happens to fall relative to its (often multi-line) question's own wrapped lines: landing on the exact y of a *later* wrap line merges the digit into that line's text (already-existing `CELL_GAP_RATIO`/`needsSpace` gap logic treats the 14-unit gap as an ordinary word space, not a cell/column boundary); landing on no wrapped line's y at all leaves it as its own isolated one-character line that then gets spliced into the middle of the same paragraph once wrapped lines are joined. Fixed with `isIsolatedRowNumber`/`stripRowNumberContinuationPrefix`: both symptoms are recognized off the same signal — this line's own x is meaningfully further left than the document's real body-text margin (`bodyX`), which no ordinary sentence in this document ever is. An isolated bare-digit line at that position is now folded into `hasListMarker`/`hasExplicitBulletMarker` (so it starts a fresh list item, exactly like question 1 and 2 already did by lucky y-alignment); a merged line whose digit prefix is followed by a lowercase word has that leading "N " read off before classification.

**A second, real, honestly-unresolved symptom surfaced by fixing the first one**: once the stray digit is correctly removed from a mid-sentence position, nothing is left to mark that a new, independent question started there — `paragraphBuffer` (unlike `bulletBuffer`/`headingBuffer`) has no sentence-boundary awareness at all, and just keeps absorbing consecutive "body"-classified lines until literally any other kind of line interrupts it. For two adjacent questions whose own row-number never lines up with either one's own first physical line (question 8 and 9 in this specific document), the two questions' text now merges into one continuous paragraph with no signal at all that they're actually separate. A general fix (giving `paragraphBuffer` the same "stop once sentence-complete, and the next line looks like a new sentence" check `bulletBuffer` already has) is a broader, higher-risk change — normal multi-sentence prose paragraphs legitimately need to *stay* merged, so this isn't attempted here without more dedicated testing time than this session had. Documented as open, real, follow-on work rather than claimed fixed.

**The utility bills (`202507_13957414.pdf`, `202606_13957414.pdf`) are a more extreme, real example of the already-identified, already-deferred column-aware-clustering gap (Feature 69)** — a dense, multi-column invoice grid (meter-reading tables, an aged-balance table with 6 columns, a property-info panel, a township panel) reads with several genuinely separate small tables/panels glued into single lines (e.g. `"Township — KLEINWINDHOEK/KLEINWINDHOEKBLOCKS/LUD — 0 — 0 — 0 — 0"`). This is the same root cause already scoped as future work in Feature 69, now with a third real document as concrete evidence for why it's worth prioritizing. The affidavit (`SS_Cohabitation Declaration.pdf`) was checked and found to already extract reasonably — a genuine fill-in-the-blank legal template, the blanks (`__________`) and full sentence structure are preserved; there isn't meaningfully more structure to recover from a one-paragraph form like this beyond what plain text can carry.

### How it was validated

`npx tsc --noEmit`, `prettier --write`, and a full corpus re-run (all 47 files now in `TestDoc/`, the new financial/legal documents included) — zero extraction failures, and every table's own count across the whole corpus stayed the same (confirming the fix doesn't touch anything `detectTableRows` already handles). The specific root cause was confirmed via a direct raw `pdfjs-dist` item dump of the exact affected page (x/y read straight from pdf.js, not inferred) before writing the fix. Re-dumped the questionnaire after the fix: the mid-sentence stray digits are gone, and isolated row-numbers now correctly start their own list item (`"- 1 How do you rate your willingness..."`, `"- 9 management and most experts..."`). The remaining question-8/9 merge was confirmed and left honestly documented above, not silently claimed fixed.
...
## Feature 74: evaluated and rejected three named AI model/browser options (Phi-4-Mini, GPT4All, "Sigma Browser"); Chrome's built-in Gemini Nano added as an optional, feature-detected fourth chat-model choice; resumable, interruption-resilient model downloads

The user asked whether four specific AI options could be added to the existing on-device chat model, subject to the app's real constraints (offline-capable, browser-based, must not lag or crash the tab), and separately asked for model downloads to survive a device turning off, an app/tab switch, or a platform change instead of restarting from zero. Researched all four options directly (real GitHub issues, real Hugging Face file listings, real API docs) before writing any code — same discipline as Feature 62's PocketPal AI evaluation.

**Phi-4-Mini — rejected.** A community ONNX/WebGPU browser build nominally exists on Hugging Face, but is actively broken in Transformers.js today (a real, open upstream GitHub issue: a session-creation error reading the model's external `.onnx_data` file, WebGPU execution reported unreliable). Even once that's fixed upstream, a real quantized (q4-class) build is ~2.3-2.6GB — 2-2.5x the size of Gemma 3 1B, the one model this project has already evaluated and rejected for real device crash reports (Features 61/63/64). A bigger, currently-broken model is directionally worse on every axis this project actually cares about, not a step up.

**GPT4All — rejected, not a tradeoff question but a category mismatch.** It's a native desktop application (Python/C++/Qt) built around `llama.cpp` bindings, with its own installer — no WASM build, no browser SDK, nothing embeddable in a web page exists anywhere. "Add GPT4All to a browser PWA" isn't buildable at any quality bar; there's no library to import.

**"Sigma Browser" — rejected, same category mismatch.** A real, confirmed product (Sigma Private AI Browser) — but it's a standalone, installable, Chromium-based *browser application* end-users browse with, bundling its own local model ("Eclipse"). No third-party developer SDK or API was found. Like GPT4All, this is the wrong category of thing entirely — an end-user browser product, not a web platform capability a website can integrate with.

**Chrome's built-in Gemini Nano (the Prompt API, `LanguageModel.create()`) — added, as an optional fourth model choice.** This one is real and genuinely usable: Chrome manages the model itself (this app never downloads or ships any model bytes for it), so there's zero size/lag cost to the app's own bundle or tab memory. But it's Chromium-desktop-only — **no Android Chrome, no iOS Safari at all** — and this project's actual audience (NFR10: budget Android phones) means this covers a real but small minority of users, not a general replacement. Implemented as `src/lib/ai-nano.ts`, kept deliberately separate from `ai-chat.ts`'s existing `CHAT_MODELS` map (Gemini Nano has no `pipeline()`, no dtype, no Cache Storage entry, no Web Worker — forcing it into the existing `ChatModelInfo` shape built for Transformers.js models would corrupt that type for no benefit). `askChatModel()` — the one entry point every caller (assistant chat, collection chat, quiz-gen) already uses — gets a single new branch: if Gemini Nano is selected, generate via the new main-thread function instead of routing through the existing worker. The worker itself (`ai.worker.ts`/`ai-worker-client.ts`) is completely untouched. Surfaced in `settings.tsx` as one more model card, rendered only when `isGeminiNanoSupported()` resolves true (silently absent everywhere else — Firefox, Safari, Android, unsupported Chrome versions — no dead option, no error), gated behind the exact same `AlertDialog` confirmation pattern already built for Gemma 3 1B, with honest copy about it being Chrome-managed, multi-GB, and desktop-only. The default stays SmolLM2 for everyone, on every platform, unchanged.

**Resumable downloads — the real fix for a real, previously-honest-but-unresolved gap.** `wake-lock.ts`'s own comment already named the correct fix (Background Fetch API) as un-built; `ChatModelDownloadPrompt.tsx` told users outright that switching apps or locking the screen "can interrupt the download and restart it from zero." Root cause: Transformers.js's own internal download/cache logic has no resumability — an interrupted fetch is just gone. Found, by reading `@huggingface/transformers`'s own source (not guessed from the type declarations alone) that its `env` object exposes a genuinely pluggable `env.fetch` override point — `getFile()` calls `env.fetch(url, { headers })` and its own progress-reporting code just reads `Content-Length` and streams the response body, nothing else. This means a resumable-download layer could be built as a **custom fetch override** with zero forking of Transformers.js and zero changes to its existing Cache Storage persistence — a fully-assembled file still flows through that exact unmodified path once complete.

New `src/lib/resumable-fetch.ts`: `installResumableDownloads(env)` (called once from inside `loadChatModel`/`loadSummarizerModel`, right after the dynamic import, before the first `pipeline()` call) installs an override that fetches each model file in chunks via HTTP `Range` requests — probing first to confirm the server actually honors `Range` (Hugging Face's CDN does; a plain single-fetch fallback covers any host that doesn't) — persisting every received chunk to a new IndexedDB table immediately, not just buffered in memory, keyed by `(url, chunkIndex)`. A single flaky chunk gets one local retry (the same `err instanceof TypeError`-gated convention already used everywhere else in this codebase for a real network-failure signal) before propagating a genuine error. Once every chunk for a URL is present, they're assembled into a real `Response` with the correct total `Content-Length` and a stream that reads the stored chunks back in order — this is exactly what the existing progress pub/sub in `ai-model.ts`/`ai-chat.ts` already expects, so the progress bar and finalizing-state logic in `ChatModelDownloadPrompt.tsx` needed zero changes. A tab closed mid-download now only ever loses the one chunk that was actually in flight at that instant (a few MB, not the whole file) — IndexedDB persists independent of the tab's own lifetime, so reopening the tab and re-triggering the same model load resumes from the last saved chunk boundary instead of starting over. New Dexie tables (`partialDownloadChunks`, `partialDownloadMeta`) added to the existing device-wide `DeviceDB` via a new `version(3)` bump — a partial download is a fact about the device, not the signed-in account, matching how the existing model-cache-status flags already work.

`wake-lock.ts`'s comment corrected — it no longer needs to claim it's the only thing standing between a locked screen and a from-scratch restart; that's handled independently now. The "restart from zero" warning (duplicated in three places — `ChatModelDownloadPrompt.tsx` plus two separate copies in `settings.tsx`, despite one of those files' own comment claiming it's shared) is replaced with honest post-fix copy: switching apps or locking the screen is fine, the download resumes; keeping the tab open just finishes it fastest.

**Explicitly not attempted this session, and why**: the stronger guarantee — surviving the *browser* being fully quit, not just backgrounded — needs the Background Fetch API, which is Chromium-only (Firefox has no plans to implement it; Safari/iOS has zero support). It would also need its own dedicated integration spike (confirming the exact Cache Storage key format Transformers.js's `FileCache` expects, so a Background-Fetch-completed download actually lands somewhere `pipeline()` will find it) before it could be trusted. Deferred as a real, scoped, lower-priority phase-2 item, not silently dropped — the resumable-fetch work above already delivers the overwhelming majority of the practical benefit (surviving a tab close, not just a backgrounded tab) for every browser, not only Chromium.

### How it was validated

`npx tsc --noEmit`, `prettier --write` across every touched/new file. The resumable-download logic was tested against a small same-origin fixture (not a real multi-hundred-MB model, for iteration speed), using `page.route(pattern, route => route.abort("internetdisconnected"))` to simulate a genuine mid-download interruption — this project already documented (Feature 45) that `context.setOffline()` doesn't reliably reach worker/fetch-level requests, so the route-abort pattern was used deliberately, not `setOffline`. Confirmed: a resumed download only re-requests the chunks it hadn't received yet (not a byte-zero replay), the final assembled file is byte-identical to an uninterrupted control download, `isModelCachedForOffline()` reports true afterward, and a server that ignores the `Range` header correctly falls back to a single whole-file fetch rather than corrupting the result. A real tab-close-and-reopen (not just a simulated route abort) was also tested, since that's the literal originally-reported symptom. **Honestly flagged, not glossed over**: this sandbox testing proves the logic is correct against a controlled fixture, but the actual real-world failure this was built to fix (mobile screen lock during a genuine multi-hundred-MB Hugging Face download) still needs confirming on a real device before calling it fully fixed — matching this project's own standing refusal to claim a device-reported bug fixed without real-device confirmation (see Features 63/64's own history of a "fix" that turned out wrong on first real-device retest). Gemini Nano's settings.tsx card was verified to render only when feature-detected and stay silently absent otherwise (mocked via `page.addInitScript()` for both present/absent `LanguageModel` global) — real end-to-end generation via an actual Chrome build with the Prompt API enabled is environment-dependent and wasn't available to test directly in this sandbox, the same honest caveat this project already uses for WebGPU.

---

## Feature 75: Background Fetch (Phase 2) — AI model downloads now keep running after every tab closes; fixed a real "no live progress during download" bug in Phase 1

Direct follow-up to Feature 74's Phase 1, done the same session: the user explicitly asked for downloads to keep going "even when the phone/device/tab is closed, after the user left the app" — a materially stronger guarantee than Phase 1's "resumes where it left off," which only helps once the user *reopens* the app. Phase 1 itself was originally scoped to ship first with Phase 2 (Background Fetch) as an optional, lower-priority layer on top; this session built that layer.

**What Background Fetch actually buys, honestly scoped**: it hands the download off to the browser itself (via the service worker), so it keeps running independent of any open tab — surviving a tab close, and on platforms that support it, surviving the whole app being quit. Real support: Chromium desktop **and Android Chrome** (this app's actual NFR10 target audience) — not Firefox, not Safari/iOS at all. Gated behind `BACKGROUND_FETCH_MIN_BYTES` (5MB) so tiny files (`tokenizer.json`, `config.json`) don't each spawn their own native download notification — those still go through Phase 1's manual chunking, which is plenty fast for something that small.

**Design, and why it sidesteps the spike the Feature 74 plan called for**: the original plan flagged a required pre-spike into `@huggingface/transformers`'s `FileCache` (`cache.js`) to learn its exact Cache Storage key format, so a Background-Fetch-completed download would land somewhere `pipeline()` could find it. Building it, a simpler design avoided that entirely: instead of trying to write into transformers.js's own cache format, the service worker's `backgroundfetchsuccess` handler (new, in `public/sw.js`) persists the completed file directly into the *same* IndexedDB tables Phase 1 already uses (`partialDownloadChunks`/`partialDownloadMeta`, via the raw IndexedDB API — a service worker's own bundle doesn't import Dexie) — as a single whole-file chunk. The next `resumableFetch(url)` call (whether from the same tab or a freshly reopened one) just finds it already complete and serves it, exactly like a Phase-1 manual download would. transformers.js's own `useBrowserCache` write still happens afterward, completely unaffected, same as before. A new `backgroundFetchStatus` table (`db.ts`, additive `version(4)`) records failure/abort so the page-side wait loop (`waitForBackgroundFetchOutcome`, a Dexie `liveQuery`) has a definitive signal to stop waiting on, regardless of whether the tab that started the fetch is even still open to receive it.

**A real, previously-unnoticed bug in Phase 1 itself, found and fixed while building this**: Phase 1's `downloadResumable` fetched every missing chunk into IndexedDB in a loop, then *only afterward* built the stream and handed it to transformers.js — meaning transformers.js's own `progress_callback` (which only starts counting once it reads the response body) never saw any activity until the entire real download had already silently finished, then jumped straight from ~0% to 100%. The existing tests never caught this because they only checked final byte-identity and chunk counts, never the *live* percentage. Fixed by adding a separate, file-name-keyed progress pub/sub (`subscribeDownloadProgress`/`notifyProgress`) that both the manual-chunk loop and the Background Fetch path feed in real time (the latter via the native `BackgroundFetchRegistration`'s own `progress` event), consumed by `loadChatModel`/`loadSummarizerModel` alongside transformers.js's own callback into the same existing UI.

**A real failure mode found and guarded against while testing this in-sandbox**: in this project's actual Playwright/Chromium test environment, a registered Background Fetch was observed to sit at `downloaded: 0` indefinitely with no `progress`/`success`/`fail` event ever firing — whether this is specific to an automated/headless-style browser context or a genuine rare browser-level failure mode wasn't determined, but either way, hanging forever on it would be a real regression versus Phase 1's already-working fallback. Added `BACKGROUND_FETCH_STALL_TIMEOUT_MS` (20s): if zero bytes have moved in that window, the attempt is treated as stuck and abandoned in favor of Phase 1's manual chunking, guarded so the (still-running, now-ignored) background attempt can't later race the manual path over the same IndexedDB rows if it does eventually resolve.

**Copy updated**: `DOWNLOAD_RESILIENCE_NOTE` (the one shared string behind `ChatModelDownloadPrompt.tsx` and both `settings.tsx` download sections) now honestly reflects the new, stronger guarantee where it's real, without overclaiming it everywhere: "on most Chrome-based browsers the download keeps going in the background and finishes on its own... elsewhere, it picks up right where it left off."

### How it was validated

`npx tsc --noEmit`, `prettier --check`, `eslint` all clean across every touched file. Rebuilt the same kind of temporary same-origin fixture + dev-only harness route Feature 74 used (deleted afterward, same as that session), against the real running Vite dev server:
- Confirmed the fixed live-progress bug: the manual-chunking path now reports 4 real progress events across the download instead of jumping straight to 100%.
- Re-ran all of Feature 74's original regression tests (uninterrupted download, interrupted-then-resumed without re-fetching completed chunks, real tab-close-and-reopen resume, no-Range-support fallback) with `BackgroundFetchManager` explicitly removed from the page to isolate the Phase 1 path — all still pass byte-identical, confirming the refactor into a shared `streamStoredChunks` helper (now reading however many chunks actually exist per URL, rather than assuming fixed-size pieces — needed since a Background-Fetch-completed file is stored as one single chunk, not many `CHUNK_SIZE` ones) didn't regress Phase 1.
- Confirmed the "unsupported browser" path (`BackgroundFetchManager` absent) skips straight to manual chunking with no delay, and confirmed the stall-timeout correctly falls back to manual chunking (with a logged reason) when a registered fetch never progresses — this is precisely what was observed happening in this sandbox's own Chromium build every time, both headless and headed.

**Honestly flagged, not glossed over**: this sandbox's Playwright/Chromium build never actually completed a real Background Fetch — every attempt sat at zero progress until the new stall-timeout gave up and fell back to Phase 1. Whether that's specific to this automated browser context or a genuine rarer real-world failure mode is unknown; either way, the *fallback* behavior was directly exercised and confirmed correct, but the actual "close the tab, background fetch keeps running, reopen later and it's already done" happy path — the literal feature the user asked for — could not be positively confirmed end-to-end in this sandbox, only reasoned about from the API's documented contract and the SW handler's own logic. This needs real-device/real-browser confirmation before calling the core promise fully proven, matching this project's own standing discipline on claims like this (see Feature 74's own equivalent caveat, and Features 63/64's history of a "fix" that only turned out wrong on real-device retest). Offline AI *inference* (as opposed to the download itself) was not re-tested end-to-end this session — verified instead at the code level: `@huggingface/transformers`'s own `loadResourceFile` (`hub.js`) checks its cache and returns a hit *before* ever calling `getFile()`/`env.fetch()`, so once a model is cached, this app's own resumable-fetch/Background-Fetch code is structurally never invoked again regardless of network state — this was already true before this session's changes and remains unaffected by them.

---

## Feature 76: real-device bug — Gemini Nano falsely reported "ready" and silently used on an Android phone, breaking offline chat

The user tested the deployed app on their actual Android phone and reported two things together: chat "can't be used offline" (app shell opens fine offline, but asking the AI a question needs a connection), and the Settings page showing the Gemini Nano card as available — "Ready on this device" / "Gemini Nano ready · used automatically" — directly contradicting Feature 74's own researched claim that Gemini Nano is Chromium-desktop-only, no Android at all.

**Root cause**: `ai-nano.ts`'s `isGeminiNanoSupported()` blindly trusted `LanguageModel.availability()`'s answer with no platform check of its own. On this real phone, Chrome's `LanguageModel` global was present and `.availability()` self-reported "available" — so the settings card showed up, got auto-selected, and `askChatModel()` routed every chat request through `generateChatViaGeminiNano()` instead of the actual offline-capable SmolLM2/Gemma path running in this app's own Web Worker. Whatever Gemini Nano does on this Android build, it isn't a genuine fully-offline on-device model the way this app's pre-launch research (and the desktop Prompt API) assumed — real-device testing is the more authoritative source here, not the API's own self-report.

**Fix**: `isMobilePlatform()` (new, `ai-nano.ts`) checks `navigator.userAgentData.mobile` (falling back to a `userAgent` regex) and forces `"unavailable"` outright on any mobile platform, *before* ever asking the browser — a deliberate override of `availability()`'s answer, not an additional condition alongside it. Applied at both checkpoints: `isGeminiNanoSupported()` (gates whether the settings card shows at all) and `loadGeminiNanoSession()` itself (rejects outright even if some other code path reaches it). `askChatModel()` (`ai-chat.ts`) now re-checks `isGeminiNanoSupported()` live before actually using a persisted `"gemini-nano"` choice, rather than trusting whatever was selected historically — self-healing a device that already had it selected before this fix shipped by falling back to the default model and correcting the stored preference, instead of failing the student's request or requiring them to know to go fix a setting themselves. `settings.tsx` gets the same self-heal for its own displayed state, via a `useEffect` that resets the choice back to `"smollm2"` once `geminiNanoAvailability` resolves to `"unavailable"` — without it, a device with a stale persisted selection would show no Gemini Nano card in the picker (correctly hidden) but still render its status/download section below for a phantom hidden choice.

### How it was validated

`npx tsc --noEmit`, `prettier --check`, `eslint` all clean. Tested with real Playwright device emulation (Android user agent + `isMobile: true`, not just a mocked global) with `LanguageModel.availability()` mocked to return `"available"` — confirmed `isGeminiNanoSupported()` still returns `"unavailable"` on the emulated mobile context while a matching desktop context correctly still gets `"available"` (i.e., this isn't a blanket regression that breaks real desktop Gemini Nano support), and that `loadGeminiNanoSession()` rejects outright on mobile even when the mocked API would otherwise happily proceed. Separately confirmed the settings-page self-heal: pre-seeded IndexedDB with `"gemini-nano"` as the stored choice (simulating exactly the reported device's prior state), reloaded on an emulated mobile context, and confirmed the UI flips back to `"smollm2"` on its own. **Not verified**: the actual real Android phone that surfaced this bug wasn't re-tested directly (this session has no access to it) — the fix is grounded in real Playwright device emulation of the same signals (mobile user agent, a Gemini-Nano-reporting-available browser), not a live retest on the reporting user's own hardware, so the user should re-check after this deploys.

---

## Feature 77: fixed a real data-corruption race between Background Fetch and manual chunking; Gemini Nano is now the default chat model on desktop; progress dead-zone fixed; researched next-step mobile AI options

The user deployed Feature 75/76 and tested on real devices. Two things came back: Gemini Nano works well on PC and they want it as the *default* there; and on phone/tablet (and to some degree everywhere), both SmolLM2 and Gemma 3 downloads were "taking so long," "failing to install," and once Gemma 3 showed "failed," Ask AI stayed completely broken afterward even though the download seemed to eventually finish.

**Root cause, found by re-reading `resumable-fetch.ts`/`public/sw.js` fresh, not guessed**: Feature 75's Background Fetch layer races a real, browser/service-worker-owned download against a 20-second stall timeout that "abandons" it in favor of manual HTTP-Range chunking of the *same* URL — but abandoning it in the app's own code never actually stopped the real, still-running browser-level fetch. If it later succeeded anyway, `sw.js`'s success handler and the manual-chunking fallback both wrote into the exact same IndexedDB row (`partialDownloadChunks` key `` `${url}::0` `` — one with the whole file, the other with just its own first 8MB slice), corrupting the reassembled buffer. That corrupted buffer reached transformers.js's `pipeline()`, which threw an ONNX/WASM error `ai-error-classifier.ts` genuinely matches as *fatal*, permanently latching `fatalModelErrors`/`summarizerFatalError` (in-memory, never cleared except by a full reload) — every retry, including clicking Download again, instantly failed without even attempting a real download. Exactly what the user saw.

One originally-suspected second mechanism (the abandoned attempt's own success branch also consuming the stored chunks) turned out to **already have a correct guard** in the shipped code (`if (abandoned) return null` before ever reaching the chunk-reading step) — verified directly against the file rather than assumed broken; a dedicated Plan-agent review caught this before any unnecessary code churn. The real, still-live bug was purely the shared primary key between the two independent writers.

**Fix, `src/lib/resumable-fetch.ts` + `public/sw.js`**:
1. When the stall timeout abandons a Background Fetch attempt, it now calls the registration's real, spec'd `.abort()` method (fire-and-forget, safe to call even if already finished) — actually stopping the browser-level fetch instead of merely giving up on waiting for it.
2. Defense in depth: a Background-Fetch-completed file is now stored under a sentinel `chunkIndex: -1` (manual chunking's own loop only ever produces `0, 1, 2, ...`, so this can never collide) instead of the colliding `0`. `streamStoredChunks` now prefers a whole-file chunk over any partial ones if both ever exist, making the corrupted-concatenation failure mode structurally impossible rather than just less likely.
3. Added an integrity check: `streamStoredChunks` now verifies the assembled byte length matches the expected total before closing. A mismatch clears the corrupted IndexedDB rows and throws a new `DownloadIntegrityError`, whose message is deliberately checked against every `ai-error-classifier.ts` fatal pattern to guarantee it can never latch a permanent failure — this also fixes devices with rows already corrupted from before this fix ships, with no separate migration needed: every real caller path already funnels through this one function, so the next attempt against a bad URL just gets a clean slate instead of repeating the corruption.
4. Both download hooks (`useDownloadChatModel`, `useDownloadAIModel`) now surface the real error message (new `errorMessage` field) instead of a generic "check your connection" string that was actively misleading for a failure that had nothing to do with connectivity.

**Progress "dead zone" fixed**: a Background Fetch attempt can spend its full 20-second stall window with zero progress events — which happens for essentially every real model file — and the existing "still working" stall-detector only ever armed once progress reached 90%, so a fresh download could sit with no feedback at all for up to 20 seconds, reading like the app had frozen. Fixed by arming the same, already-tested 4-second stall timer from t=0 and widening its gate to also cover exactly 0% (the 1-89% "normal slow trickle" zone keeps its existing, deliberately quiet behavior unchanged). Copy at all three download-status render sites now distinguishes "Preparing your download…" (zero-progress case) from the existing "Finishing up, almost there…" tail-end message.

**Gemini Nano is now the default chat model on desktop** (SmolLM2 stays the default on mobile, unchanged): new `getDefaultChatModel()` (`ai-chat.ts`) picks based on `isMobilePlatform()` (now exported from `ai-nano.ts`) — deliberately synchronous, so it works in `useChatModelChoice`'s `useState` initializer without new async complexity. Wired in at exactly two of three `DEFAULT_CHAT_MODEL` usage sites, not all three: `getSelectedChatModel()`'s fresh-device fallback and `useChatModelChoice`'s initial state both switch to the new platform-aware function, but `askChatModel`'s self-heal call (fires once the *async*, real `isGeminiNanoSupported()` check has just confirmed Gemini Nano doesn't actually work) deliberately keeps targeting the static `"smollm2"` — if it re-derived the platform default instead, a desktop browser that fails the real availability check but passes the synchronous mobile check (desktop Firefox, desktop Safari, an unflagged Chrome) would self-heal right back to the very model that just failed, every time, never landing on something that actually works. Self-healing from a confirmed-broken selection must always land on the one universally-safe model, not "the platform default," which can itself still be wrong.

**Mobile in-browser AI — researched, not built this pass**: checked what's changed since Feature 74's research. WebGPU now has real mobile coverage (Chrome 121+/Android 12+ with Qualcomm/ARM GPUs; Safari on iOS 26) that didn't exist when this app chose to stay WASM/CPU-only. Google also shipped LiteRT.js and LiteRT-LM.js (July 9, 2026) — a new JS runtime running `.tflite` models in-browser via WebGPU on both Chrome and Safari. Two concrete, evidence-backed candidates for a dedicated future session: a feature-detected WebGPU execution backend for the existing SmolLM2/Gemma 3 models (falling back to today's WASM path), and LiteRT.js/LiteRT-LM.js as a possible alternative runtime for mobile performance specifically — bigger and riskier, needing its own real spike before any commitment, same discipline already applied to Phi-4-Mini/GPT4All/PocketPal AI. Not implemented this pass; recorded here so it isn't lost.

### How it was validated

`npx tsc --noEmit`, `prettier --check`, `eslint` all clean. Real Background Fetch still doesn't reliably progress in this sandbox's Chromium build (confirmed again this session — a registered fetch sat at zero `downloaded` the entire time, every attempt), so the race itself was proven by simulating it directly at the IndexedDB level rather than relying on a genuine completed fetch: a temporary dev-only fixture + route (deleted before committing, same pattern as Features 74/75) ran the real manual-chunking path while, at the same instant, directly performing the exact raw-IndexedDB write `sw.js`'s success handler performs for the same URL — confirmed the final assembled file is still byte-identical to the original (SHA-256 match), proving the disjoint-key fix actually prevents the corruption rather than just reducing its odds. The integrity check was tested by pre-seeding a deliberately size-mismatched whole-file chunk: confirmed `DownloadIntegrityError` throws, `isFatalCategory(classifyModelError(err))` is `false`, and the corrupted rows are gone afterward (a clean slate for the next attempt). The `.abort()` call was confirmed via an instrumented mock `BackgroundFetchManager` that never progresses (matching this sandbox's own real, observed behavior) — verified `.abort()` fires exactly once when the real 20-second stall timeout elapses, and that the download still completes correctly via the fallback. Platform-conditional default-model resolution was verified with real Playwright device emulation (desktop vs. Android), including confirming the self-heal target stays `"smollm2"` (not a loop back to `"gemini-nano"`) when the async availability check fails on a desktop browser with `LanguageModel` forced absent. The progress-dead-zone fix was verified by truth-table check of the widened stall-timer gate (arms at exactly 0% and ≥90%, stays quiet 1-89%, matching the intended change precisely) rather than driving a real multi-hundred-MB download end-to-end through the UI, which wasn't worth the time cost for what is a small, mechanical, easily-verified boolean-logic change. **Not verified**: the actual real device(s) that surfaced this bug weren't re-tested directly (no access to them this session) — the fix is grounded in direct IndexedDB-level race simulation and code-level reasoning, not a live retest on the reporting user's own hardware. The user should re-check on their phone/tablet and PC after this deploys.

---

## Feature 78: real bug — Gemini Nano as the new desktop default broke every "Ask AI" surface; SSR hydration mismatch; honest proof-of-concept framing added to Terms and the download prompt

The user tried the Gemini Nano default on their PC and hit `Error: loadChatModel doesn't support Gemini Nano — use loadGeminiNanoSession instead` right on the Ask AI screen, even though Settings showed Gemini Nano as "Ready on this device." Confirmed SmolLM2 works "wonderfully offline" on desktop (positive, unprompted confirmation worth keeping).

**Root cause**: Feature 77 made Gemini Nano the default chat model on desktop, but missed that `useChatModelStatus()` — the flag every one of this app's five chat surfaces (`assistant.tsx`, `documents.$docId.chat.tsx`, `documents.collections.$collectionId.chat.tsx`, `courses.$moduleId.chat.$docId.tsx`, `courses.$moduleId.chat.index.tsx`) and the quiz-generation gate (`courses.$moduleId.read.$docId.tsx`) all used to decide "is AI ready" — can never report `"ready"` for Gemini Nano at all (`isModelCachedForOffline` always returns `false` for it, by design, since Chrome — not this app — owns that model). So a Gemini-Nano-default desktop user with no cloud key saw `<ChatModelDownloadPrompt />` forever, and its "Download assistant" button unconditionally called the transformers.js-only `useDownloadChatModel()`, which throws exactly that error the instant it resolves to `"gemini-nano"`. The same blind spot existed, at lower severity (a permanently-disabled button, not a thrown error), in the quiz-generation gate.

**Fix**: a new shared hook, `useChatEngineReadiness()` (`use-ai-chat.ts`), replaces five copies of an identical 5-line `chatReady` computation across all five chat routes plus the quiz gate — now correctly counting Gemini Nano as ready when `useGeminiNanoAvailability()` resolves `"available"`, alongside the existing on-device-model-ready and cloud-ready checks. `ChatModelDownloadPrompt.tsx` itself is now Gemini-Nano-aware: it branches on the selected model and, for Gemini Nano, shows its own Chrome-managed download flow (reusing `useDownloadGeminiNano`, the same hook settings.tsx's own card already uses) instead of ever calling the wrong download function. It also self-heals inline (mirrors settings.tsx's own equivalent effect) since this component is reached from five different surfaces that don't all pass through Settings first.

**A second real bug found while testing the first fix**: making `useChatModelChoice`'s initial state platform-dependent (`getDefaultChatModel()`, which checks `navigator`) broke server-side rendering for mobile users specifically — `navigator` doesn't exist during SSR, so the server-rendered HTML always assumed desktop ("gemini-nano"), while a real mobile client's hydration pass correctly detected mobile ("smollm2"), a genuine mismatch React reported outright (`Hydration failed because the server rendered text didn't match the client`). Fixed by reverting the initial `useState` to the static, platform-independent `DEFAULT_CHAT_MODEL` ("smollm2", identical on server and client) — the existing `useEffect` that resolves the real stored/default choice already runs client-only, after mount, so it corrects to `"gemini-nano"` on desktop within one tick with no SSR involved at all. Desktop users never hit this (the server's conservative guess matched their real platform anyway), which is why this wasn't caught until testing the mobile case specifically.

**Honest framing added, at the user's explicit request**: the on-device AI is framed as a working proof of concept — genuinely demonstrating that free, offline AI is possible, not a finished product, with device-dependent reliability actively being investigated, needing patience — in both `legal.tsx`'s "AI features" section (expanded, dated 2026-07-28) and a new shared note in `ChatModelDownloadPrompt.tsx` (shown on every chat surface, for both the on-device and Gemini Nano branches) pointing to the already-existing "connect a free cloud AI key" flow in Settings for anything needing a dependable answer.

### How it was validated

`npx tsc --noEmit`, `prettier --check`, `eslint` all clean. Verified with a temporary unauthenticated preview route (deleted before committing, same pattern as prior sessions) rendering `ChatModelDownloadPrompt` directly: on a real desktop context, it now shows the Gemini Nano branch with a working "Download Gemini Nano" button that calls the correct function with zero thrown errors (previously: an immediate crash); on a real Playwright Android-emulated mobile context, it correctly falls back to the SmolLM2 prompt with **no hydration error** (confirmed the exact "Hydration failed" React error before the second fix, and its absence after). **Not verified**: an actual completed Gemini Nano download and a real chat response through it — this sandbox's Chromium build reports Gemini Nano as genuinely installable but downloading it fully wasn't exercised end-to-end here, consistent with this project's standing caveat on Prompt-API testing. The user should confirm on their own PC that Ask AI now works without erroring.

---

## Feature 79: real bugs — offline stuck-loading on unvisited Ask AI threads, sign-out-after-offline, stacking "new version" toasts — plus missed AI honesty copy and creator info

The user reported several things after using the deployed app for a while: a stale/stacking "A new version of eLearn is available" toast; the Research Study Consent screen missing the same AI-honesty framing added to Terms and the Ask AI download prompt; switching into an Ask AI thread while offline sometimes showing the loading state forever with nothing rendering, "especially in the Ask AI tab"; sometimes being signed out after a period offline and having to log back in; and a request to add LinkedIn/freelance/contact info to the About page.

**"New version" toast stacking, `src/hooks/use-sw-update.ts`**: `toast.message()` was called with no stable `id` and `duration: Infinity` — a real, legitimate `controllerchange` event fires once per actual `sw.js` content change (which happened on nearly every recent deploy this session), and each one created its own untracked, never-auto-dismissing toast, so testing across several deploys in a short window left multiple stacked. Fixed with a stable `id` so a later firing replaces the existing toast instead of stacking a new one.

**Missing AI honesty on the consent screen, `src/components/ResearchConsentGate.tsx`**: added the same proof-of-concept framing already in `legal.tsx`/`ChatModelDownloadPrompt.tsx` — the on-device AI is early, device-dependent, may have bugs that need more research/time/effort to fix — plus a direct `mailto:` link so a participant can report something not working, since that's genuinely useful data for the study, not just a complaint.

**Offline stuck-loading, especially nested Ask AI threads**: root-caused by reading TanStack Router's own internals (no framework-level loader timeout exists; a match stays `isFetching` until its own promise settles) cross-referenced against this app's existing safeguards. `src/lib/modules-api.ts`'s 6s-timeout+cache-fallback already covers every route loader that touches the network. The real gap: `src/hooks/use-precache-routes.ts` (client-side chunk warming) and `public/sw.js`'s `PRECACHE_ROUTES` (HTML warming) both list the general `/assistant` tab, but not the four *nested* Ask-AI chat routes reachable from a specific material/collection/module (`/documents/$docId/chat`, `/documents/collections/$collectionId/chat`, `/courses/$moduleId/chat/`, `/courses/$moduleId/chat/$docId`) — each just as reasonably "the Ask AI tab" to a student who reaches it from a course material rather than the bottom nav. Opening one of these for the first time this session while offline left its route chunk never fetched, so the router had nothing to resolve to and never settled. Fixed by adding all four (with placeholder params, the same pattern already used for `/documents/$docId`) to `use-precache-routes.ts`'s warming list. Also found and fixed a related gap while in there: `public/sw.js`'s own `precacheRoutes()` had no timeout on its route/asset fetches at all (unlike every other network call in this app) — a slow connection right after sign-in could leave this precaching mechanism itself silently incomplete on the exact condition it exists to protect against. Added a 6s timeout, matching `modules-api.ts`'s own convention.

**Sign-out after being offline, `src/hooks/use-auth.tsx`**: root-caused by reading `@supabase/auth-js@2.110.7`'s own source. `getSession()`'s internal `__loadSession()` reactively tries to refresh the access token once it's *actually* expired (not just inside the 90s proactive margin) — and if that refresh fails for *any* reason, including a plain offline network error, it returns `{ session: null }` to the caller, even though it deliberately does **not** clear the persisted session in storage on that path (only a genuine, non-retryable auth error does that). `use-auth.tsx` trusted that return value unconditionally, so a fresh mount (a reopened tab or relaunched app, not one that stayed open) occurring after a long-enough offline stretch (roughly once it exceeds the access token's real lifetime, ~1hr by default) saw `user` forced to `null` and had to sign in again, even though the actual refresh token sitting in storage was never invalidated. Fixed with a deliberately best-effort fallback: when `getSession()` reports no session while `navigator.onLine` is false, read the same storage key supabase-js itself writes to directly, and use its `user` field if present, rather than forcing a sign-in screen. Never trusted for anything security-relevant (the server's own RLS/JWT validation is the real authority regardless of what this client-side value shows) — wrapped so any wrong assumption about the storage format just falls through to the existing, safe behavior.

**About page, `src/routes/about.tsx`**: added a LinkedIn link (linkedin.com/in/joao-nkunku), a line noting he builds websites/web tools generally and is open to being contacted, and a direct email link (nkunkujoao01@gmail.com), alongside the existing GitHub follow card.

### How it was validated

`npx tsc --noEmit`, `prettier --check`, `eslint` clean (one pre-existing, unrelated `react-refresh` warning on `use-auth.tsx` for exporting a hook alongside a component — present before this session's changes). The offline-auth fix's two real halves were each independently confirmed against the actual running dev server rather than assumed: with the device forced offline (`context.setOffline(true)`) and a plausible, already-expired supabase-js v2 session blob seeded under this project's real default storage key, `supabase.auth.getSession()` was confirmed to genuinely return `{session: null}` in that exact scenario (the real trigger the fix targets), and the exact storage key + parsing this fix uses was confirmed to correctly recover the real stored user from that same blob. **A full page-reload-while-offline test could not be driven end-to-end in this sandbox**: Vite's dev server relies on hundreds of unbundled, on-demand-served files (`/@id/virtual:...`, raw `/src/...` modules) that this app's service worker was never designed to cache — a real production build's single hashed, pre-cached bundle wouldn't hit this, but this sandbox has no production-preview setup to confirm that directly; the two verified halves above are what the fix's correctness actually depends on regardless. The five new/changed `preloadRoute` calls in `use-precache-routes.ts` were confirmed to resolve without error against the real router instance for each of the four new route paths with their placeholder params. The toast-dedup and consent-copy changes are straightforward enough that direct code review was the verification; not independently re-screenshotted.

---

## Feature 80: research page now shows a real name/student number instead of "User_XXXX" for identified respondents; corrected an outdated privacy-policy claim

Root cause of the "0 agreed, 0 declined, no submissions" the user saw right after deploying Feature 73's anonymity work: `0042_research_optional_identity.sql` (adds `user_id`/`full_name` to `research_consent`/`research_survey_responses`) was written and committed, but never actually run against the live Supabase database — every research query failed outright with PostgREST's `42703` (undefined column), which reads as "no data" rather than a real error in the UI. Confirmed the real data was there the whole time; the query just couldn't reach it. Fixed by having the user run the migration directly via the Supabase SQL Editor (no direct database credentials available in this environment, only API-level keys) — confirmed live afterward.

Once visible, every respondent still showed as an anonymous "User_XXXX" tag, even ones who'd agreed to be identified — asked for by name specifically. Root cause: these visible rows all predate Feature 73 (3-4 days old), from before the identified-by-default capture existed at all, so `full_name` is genuinely null for them, not anonymous by choice — nothing to fix there, just how the timeline landed. Separately, the user asked for two real improvements: show a student number too when available, and for an identified respondent, show the name (and number) *instead of* the `User_XXXX` tag, not alongside it.

**Student number, `research-study.ts`/new `0043_research_student_number.sql`**: there's no `profiles.student_number` column at all — the only place a student number is ever recorded is baked into the synthetic login email the NUST-student-number sign-in method uses (`<studentnumber>@nust-student.invalid`, see `moodle-server.ts`'s `studentNumberToEmail`). Added `studentNumberFromEmail()`, extracting it back out of `user.email` at submission time (same snapshot-not-live-join reasoning as `full_name`), null for anyone who signed in with Google or a plain email/password instead. New nullable `student_number` column on both tables, threaded through `ResearchConsentRow`/`ResearchSurveyResponseRow` (`supabase.ts`), `ResearchSubmission`/`useResearchSubmissions` (`use-platform-analytics.ts`), the CSV export, and disclosed in both the consent screen's own "Confidentiality & data storage" section and `legal.tsx`.

**Display, `admin.super.research.tsx`**: an identified respondent now shows as `Real Name (student number)` — or just `Real Name` if they didn't sign in with a student number — with the `User_XXXX` tag dropped entirely; it's shown on its own only for a genuinely anonymous respondent, where it's the only identifier that exists. Matches the user's own framing exactly: the anonymous tag is precisely what someone chose *not* to be identified by, so showing it alongside a real name undercut the point of asking.

**A real, unrelated accuracy bug found while touching this**: `legal.tsx`'s "Feedback and the research study" section still claimed the study's data "is anonymous: it is never linked to your name, student number, or account" — left over from before Feature 73 made identification the default. Corrected to accurately describe the current identified-by-default-with-opt-out design, since an accurate privacy policy matters regardless of what prompted noticing it was stale.

### How it was validated

`npx tsc --noEmit`, `eslint` clean (prettier has no SQL parser configured in this repo, consistent with every other migration file never being run through it). The three real display cases (identified with a student number, identified without one, genuinely anonymous) were confirmed against a temporary preview route with mock data, rendering exactly `"Jolyne Nkunku (223068209)"`, `"Peter Amutenya"`, and `"User_3333"` respectively — deleted before committing. **Not independently re-verified against the real, live Supabase data** (would need the new migration applied first, which is on the user to run, same as 0042) — the user should confirm real identified respondents show correctly once `0043_research_student_number.sql` is live.
...
## Feature 83: dashboard's SSR response was carrying every module's full document text for no reason — cut ~97% off it

Follow-up to Feature 82. The user re-ran Lighthouse against the real deployed `/dashboard` after that fix shipped: Performance 25→32, Accessibility 89→96, Best Practices 77→96 — the eager-precache fix clearly worked (Total Blocking Time alone dropped from 15,960ms to 1,540ms, a ~10x cut). But FCP (5.0s) and Speed Index (18.6s) were still bad, and the report's own diagnostics still pointed at "Minimize main-thread work" and a heavy "Network dependency tree."

Traced this to `fetchModules()` (`src/lib/modules-api.ts`), the loader `/dashboard`, `/courses`, `/progress`, and `/summaries` all share. Its Supabase query was `select("*, materials(*), module_quizzes(*))")` — `materials(*)` includes `content`, a JSON column holding a material's full extracted document text (`{ heading, lead, body: string[], pull }`). None of the four pages that call this ever read `.content` (confirmed with a repo-wide grep) — they only need `id`/`title`/`kind`/`pages`/`size_mb` for completion counts and size badges. Confirmed directly against the real live Supabase database (not guessed): the full query's JSON payload is **187,002 bytes**; the trimmed query (dropping just `content`) is **5,447 bytes** — a 34x reduction, entirely dead weight this page was downloading, embedding into the SSR HTML, and shipping into the hydration payload for zero benefit.

**Why it wasn't harmless to just delete outright**: `fetchModules()` also feeds `cacheModules()`, which writes the full result into IndexedDB (`deviceDb.catalogModules`) — this is what lets a student open *any* module they've merely seen in a list (not explicitly downloaded) while offline (Feature 30's deliberate design). Blindly trimming `content` everywhere would have silently broken that. Two things made it safe to trim specifically for the server-rendered case: (1) `cacheModules()` already no-ops during SSR (`isBrowser` guard — IndexedDB doesn't exist in Node), so the SSR-side fetch was never actually reaching the offline cache anyway; (2) `usePrecacheRoutes()` (Feature 82) already re-triggers each of these same routes' loaders client-side, in the browser, shortly after sign-in — which now takes over as the thing that actually warms the offline cache with full content, deferred to idle time instead of blocking the initial paint.

### What changed

- **`modules-api.ts`**: `fetchModules()`'s materials select is now conditional on `isBrowser` — full `materials(*)` in the browser (preserves Feature 30's offline guarantee exactly, unchanged), a trimmed `materials(id, module_id, title, kind, pages, size_mb, created_at)` during SSR. `Material.content` widened to `MaterialContent | undefined` to reflect this honestly; `fetchModule()` (singular, per-document detail/reading routes) is untouched and always fetches full content.
- **`courses.$moduleId.index.tsx`, `courses.$moduleId.read.$docId.tsx`**: the only two places that read a list-sourced material's `.content` directly (both are "Download for offline" click handlers) — both already only ever run against `fetchModule()`-sourced data where content is always present in practice, but the type is now honestly optional, so both handlers gained a `mat.content &&` / `doc.content &&` guard rather than lying to the type checker.

### How it was validated

`npx tsc --noEmit`, prettier, eslint clean. Ran both the trimmed and full query directly against the real, live production Supabase database (not a mock) via a standalone script using the project's real `.env` credentials: both succeed with no error, returned row shapes match expectations, and the byte counts above (187,002 → 5,447) are real, measured numbers. Grepped every reader of `Module.materials[].content` app-wide to confirm the only two call sites needing a guard were the ones fixed. **Not re-run through Lighthouse** — same reasoning as Feature 82, this environment can't reproduce the user's real mobile/Slow-4G conditions. The user should re-run Lighthouse against `/dashboard` again after this deploys; expect a real drop in FCP/SI given the payload cut, though some of the remaining gap (Vercel serverless cold starts — confirmed directly: 4.3s TTFB on a cold hit vs ~0.9s once warm — plus Slow-4G's own throttling math on a genuine ~1MB-class JS/CSS bundle) is infrastructure/lab-condition overhead this change doesn't touch.

---

## Feature 84: baseline system-architecture and UI-wireframe diagrams (docs only, no app code changed)

The user asked for the *current* system architecture and UI design captured as wireframes/diagrams they could open in draw.io, to use as the shared baseline for planning future work — not a proposal for what to build next.

Rather than guess at generic "typical PWA" boxes, both diagrams were built directly from a fresh read of the live codebase in this session: the full `src/lib/` data layer, all 43 `supabase/migrations/`, `public/sw.js`'s actual caching logic, `src/server.ts`/`src/start.ts`'s server entry points, and every route file under `src/routes/` plus the shared `MobileShell`/`AdminShell` component structure.

### What was added

- **`docs/architecture/system-architecture.drawio`** — revised twice in the same session after the user asked for "a better system architecture design." The shipped version uses real draw.io shape semantics instead of uniform boxes: an actor shape for the end user (single entry point), cylinders for the two databases (Postgres, Dexie), hexagons for background workers/schedulers (Service Worker, AI Worker, pg_cron), cloud shapes for the four external services, rounded rectangles for everything else — grouped into four color-tinted layer bands (client / Vercel / Supabase / external) via real draw.io swimlane containers (not just background rectangles). The 20 data-flow arrows are colored by *flow category* (auth=purple, content/browsing=blue, offline storage=green, AI=orange, sync/Moodle=teal dashed) independent of layer color, so a reader can trace one kind of data movement across the whole diagram — plus a two-part legend (shape key + flow-color key) explaining both systems explicitly.
- **`docs/architecture/wireframes.drawio`** — a 9-tab low-fidelity wireframe file (desktop layout only, per the user's choice), one tab per core screen: Auth (login+signup), Dashboard, Library+Module detail, Reader, Quiz+Flashcards, Assistant/Chat (incl. the model-download gate state), Documents, Profile, Settings — each laid out to match the real component structure found (e.g. Reader's chrome-less full-screen shell with a fixed bottom action bar; Profile's stacked `SettingsGroup` sections; the shared sidebar nav order Home/Library/Ask AI/Progress/Settings/Profile).
- **`docs/architecture/architecture-design-prompt.md`** — the user separately asked for a prompt they could hand to another AI/design tool to render a more polished version of the same architecture. Written as a single detailed, copy-pasteable prompt: the same layers/components/flows as the `.drawio` file, translated into explicit shape/color/typography/layout rules (no emoji, five meaning-coded shape types, orthogonal low-crossing routing, a two-axis color system, a required legend, a landscape canvas spec) — so a different tool reproduces the same diagram, not a reinterpretation of it.
- **`docs/architecture/README.md`** — explains all files represent the current, as-built system (not a proposal), how to open a `.drawio` file (draw.io desktop, app.diagrams.net, or the VS Code extension — plain mxGraph XML, no import step needed), and that they're a snapshot as of 2026-08-06, not a live-updating artifact.

### How it was validated

Both files were generated via small Python scripts (kept in the session scratchpad, not committed) rather than hand-typed, specifically to avoid id-mismatch/malformed-XML mistakes; each was parsed with `xml.etree.ElementTree` to confirm well-formed `<mxfile>` XML before being written into the repo — draw.io shows a blank canvas with no error on malformed input, so this check was necessary, not optional. For the v2 architecture diagram's swimlane-container rewrite, an extra check cross-referenced every edge's `source`/`target` attribute against the full set of defined cell ids (`0 missing refs` on the final run) — this caught a real class of mistake the first version's flatter layout didn't have: several legend shapes/labels were initially left parented to the root layer with coordinates meant to be relative to the legend container, which would have rendered them in the wrong place; fixed by making the shape/text helper functions take an explicit `parent` argument instead of hardcoding one. Not opened in an actual draw.io instance from this environment (no GUI available here) — the user should do a first open themselves to confirm the visual layout reads as intended.

---

## Feature 85: a thesis-facing system evaluation report (`docs/evaluation-report.md`) — technical results real and complete, usability-study section deliberately left as a pending placeholder

The user asked for the "evaluation report, thesis draft" item from their project timeline (Week 11–12: "Data analysis and writing of report") — an evaluation of the system combining real technical results with the app's own NUST ethics-approved usability study (SUS/TAM-UTAUT/data-efficiency), explicitly wanting real data, not invented numbers.

**A real, live-database query was attempted and correctly blocked.** To pull the real `research_survey_responses`/`research_consent`/`feedback` rows, a read-only script (service-role key, same pattern as Feature 83's verification query) was written to hit the live Supabase REST API directly — this was blocked outright by the environment's own outward-facing-network-call classifier. Rather than route around it (e.g. via some other tool that happens to have network access), this was surfaced to the user directly, who chose the correct path themselves: export the real data manually from the app's own already-built `/admin/super/research` → "Export CSV" screen (super-admin-only, exactly as it should be) and hand it back, rather than have an agent reach into a live production database unsupervised.

### What was added

- **`docs/evaluation-report.md`** — structured as: system overview (references `docs/architecture/system-architecture.drawio`), evaluation methodology (technical + usability, both described accurately against what was actually done), then two results sections:
  - **§4 Technical evaluation — fully real, pulled from `DEV_LOG.md`'s actual history**: extraction-accuracy corpus testing (25 → 40 → 55 real `TestDoc/` files across Features 65–70), real Lighthouse before/after numbers (Performance 25→32, TBT 15,960ms→1,540ms, the 187KB→5.4KB SSR payload cut), real throttled-network cold/warm load numbers (Feature 29), the two real offline-resilience bugs found and fixed, the WCAG 2.1 AA contrast fix, and the 6 real bugs found only through real-device testing (Feature 61) — plus an honest "known limitations" subsection (the unreproduced crash report, unconfirmed real-device quiz timing, the Vercel access block) rather than presenting the system as flawless.
  - **§5 Usability study — deliberately left as a clearly-marked placeholder**, not a fabricated number: explains exactly what's missing (the real SUS/TAM/data-efficiency/open-ended responses), the standard SUS 0–100 scoring formula to apply once the data lands, and the exact 3-step path to get it (sign in as super admin → `/admin/super/research` → Export CSV → hand the file back).
- §§6–7 (Discussion/Conclusion) are written to explicitly depend on §5 landing before a real verdict is drawn, rather than concluding from technical results alone.

### How it was validated

Every number in §4 was cross-checked against its exact source line in `DEV_LOG.md` (Features 29, 61, 65–70, 82, 83) and `REAL_DEVICE_TESTING.md` while writing this report, not recalled from memory — including re-reading the real SUS/TAM/data-efficiency question text directly from `src/components/ResearchSurveyModal.tsx` and the real schema/RLS design from `supabase/migrations/0025`, `0042`, `0043` so the methodology section describes the actual instrument and access model, not a generic description of "a usability study." §5 is intentionally not filled with placeholder/estimated numbers — the user was explicit about wanting real data only.

---

## Feature 86: security review (token-tampering + STRIDE) — one real fix (non-constant-time cron secret compare) plus a persisted super-admin audit log

The user asked for a review of whether session/secret tokens could be manipulated, and a STRIDE threat model of the system. Reviewed the pending PR diff first (no exploitable findings — every change in it was a hardening/bug fix, not a new risk), then traced every token path in the app (Supabase JWT handling, the Moodle `wstoken` encryption/RPC boundary, admin RPC authorization) against the codebase's own existing patterns. The architecture already does the hard part right: every privileged server action re-verifies the caller's identity via `supabase.auth.getUser(accessToken)` rather than trusting a client-supplied claim, RLS blocks self-escalation (`0030`/`0035`), and the Moodle token is Vault-encrypted and never reaches the browser. Two real, concrete gaps came out of the STRIDE pass (Spoofing and Repudiation rows) rather than the token-tampering question itself, which came back clean.

### What was added

- **`src/lib/moodle-cron-handler.ts`** — the `x-cron-secret` header check used a plain `!==` string comparison, which short-circuits on the first mismatched byte and leaks timing information about how much of a guessed secret is correct. Replaced with a hand-rolled constant-time compare (`timingSafeEqual`, walks the full length of the longer input regardless of where a mismatch occurs) rather than `node:crypto`'s `timingSafeEqual`, since this file explicitly supports both a Cloudflare Workers deploy and a Node host (see its own `readEnvVar` comment) — confirmed this project's actual Nitro preset targets Cloudflare Workers (`wrangler.json` in the build output), so a Node-only API would have been the wrong call.
- **`supabase/migrations/0044_admin_audit_log.sql`** (new) — an `admin_audit_log` table (RLS enabled, zero client grants, reads only via a new `get_admin_audit_log()` RPC gated on `is_super_admin()`, same shape as `get_all_users_admin_info`) plus a redefinition of `admin_set_lecturer_role` that inserts a row after its update succeeds. No FK on `actor_id`/`target_user_id` to `auth.users`, deliberately — an `on delete cascade` would delete the very audit row proving an account was deleted, the moment `deleteUserAccount` ran.
- **`src/lib/admin-server.ts`** — `verifyCallerIsSuperAdmin` now returns the caller's own id alongside the authorization verdict; `setUserBanned`/`deleteUserAccount` each `await` an audit insert after their privileged action succeeds (awaited deliberately, not fire-and-forget — a serverless invocation can be torn down the instant the response is sent).
- **`src/routes/admin.super.audit-log.tsx`** (new) + a `useAdminAuditLog()` hook in `use-super-admin.ts` + an "Audit log" nav entry in `AdminShell.tsx` — a read-only page listing the 500 most recent super-admin actions, resolving raw actor/target ids against the same user directory the existing "User directory" page already loads.

### How it was validated

`tsc --noEmit` and `eslint` clean on every touched file; `npm run build` succeeds. The migration itself was not applied to the live Supabase project in this session (no destructive/live-data action was taken) — it needs a normal `supabase db push` before the audit-log inserts/reads will work in production; until then `recordAdminAction` degrades gracefully (logs and continues, doesn't block the ban/delete/grant action itself).

---

## Feature 87: replaced the static WelcomeTour slideshow with an interactive, "teach by doing" onboarding tour — three real bugs found and fixed by actually driving it in a browser

Follow-up to a large onboarding/UX design brief the user pasted; scoped down (by explicit user choice) to just the first-run tour itself, not an app-wide error-state redesign. The old `WelcomeTour.tsx` was five static slides describing features in prose. The new `OnboardingTour.tsx` spotlights four real, already-on-screen elements in sequence (Dashboard's "Get" button → the featured module's open button → a material's "Open" link → the reader's "Summarise" button) and advances only on a genuine click of that real element — never a "Next" button — so the student's first action in the product is also their first onboarding step. Reaches this app's actual "aha moment" (a real, on-device AI summary) in four real actions instead of a five-slide read.

### What was added

- **`src/components/OnboardingTour.tsx`** (new, replaces `WelcomeTour.tsx`) — a calm welcome modal (what/why/what's-next, no marketing language), four spotlight steps (dimmed backdrop via an SVG mask with a cutout around the real target, a gold ring, a callout with the instruction + a "Skip tour" exit, positioned above/below the target based on available viewport space), and a plain completion modal. `data-tour="..."` attributes added to the real target elements in `dashboard.tsx`, `courses.$moduleId.index.tsx`, and `courses.$moduleId.read.$docId.tsx` — the tour finds and highlights them via `document.querySelector`, never renders a copy of its own.
- **`src/routes/profile.tsx`** — a "Replay tour" row in Settings (`REPLAY_ONBOARDING_TOUR_EVENT`, a plain `window` event) so a student can see it again without any DB round-trip; separate from `profile.onboarding_completed_at`, which still only gates the automatic first-run showing.

### Three real bugs found only by driving the actual running app (Playwright, a real signed-in test user created via the Admin API, deleted afterward — same pattern as prior features)

1. **A stale-hook race with `ResearchConsentGate`.** That component's own `useResearchConsentGate()` hook owns one component's worth of local `responded` state. Calling it a second time from `OnboardingTour` (to defer to the consent gate, since both are independent top-layer first-load gates for a brand-new student) created a second, independent copy that correctly saw the initial "not yet responded" state but never learned the gate was dismissed elsewhere — the tour stayed permanently hidden after the consent gate closed. Fixed by polling a DOM marker (`data-onboarding-gate="research-consent"` on the gate's own root element) instead of duplicating the hook's state.
2. **A lost click on step 3 (open a module → navigate).** A bubble-phase `document.addEventListener("click", …)` lost the race against the router's own synchronous re-render on navigation: the listener attached for one step got detached and replaced before the event finished bubbling to `document`, silently swallowing the click that was supposed to advance the tour. Fixed by attaching in the capture phase instead, which always fires before the target's own click handling.
3. **The step-4 callout rendering off-screen, overlapping the real "Summarise" button it was pointing at.** `animate-rise` (this app's standard entrance animation) sets `transform` via a CSS `@keyframes`/`animation` rule with fill-mode `both` — which always wins over an inline `style.transform` for the same property, for as long as it applies (here, permanently). The callout's own "flip above the target" `translateY(-100%)` (needed only when the target sits near the bottom of the viewport, as the reader's floating action bar does) was being silently discarded by the animation. Fixed by splitting the callout into an outer positioning element (owns `top`/`left`/the flip transform, no animation) and an inner element (owns `animate-rise`), so the two `transform` sources never collide.

### How it was validated

`tsc --noEmit` and `eslint` clean on every touched file; `npm run build` succeeds. Then the real app was driven end to end with Playwright against a real signed-in test user (created pre-confirmed via the Supabase Admin API, deleted afterward): login → dismissed the real research consent gate → welcome modal → all four spotlight steps, each confirmed by finding the real target element and its callout text, not a mock → confirmed the material row genuinely showed "Open" (not "Get") at step 3, proving the whole module's materials really were downloaded and cached by step 1's click, not just flagged → step 4's real Summarise click genuinely regenerated a real on-device summary (the reader's own button relabeled to "Regenerate" afterward) → completion modal → confirmed the overlay/modal fully disappears with no leftover blocking element after "Done". Also verified Escape and every step's "Skip tour" tear down cleanly. Zero real console errors throughout (the only logged error was the test harness's own deliberate service-worker-registration block, unrelated to the app). Screenshots confirm the visual result at every step, including the corrected step-4 positioning.

---

## Feature 88: magic-link (passwordless) sign-in, real SMTP set up, branded auth emails, and shipping a batch of prior uncommitted work found in the tree

The user asked for phone/SMS OTP first; reconsidered given no budget for Twilio's per-SMS cost, and switched to email magic-link/OTP instead, which uses Supabase's built-in free email sending (same path `forgot-password.tsx` already used) — no per-message cost. Also walked the user through setting up real custom SMTP (Gmail with an App Password — the dashboard had Google OAuth Client ID/Secret pasted into the SMTP username/password fields by mistake at first, which can never work; corrected to a real Gmail address + App Password) and wrote branded HTML templates (matching eLearn's actual deep-green/gold palette, replacing Supabase's generic defaults) for all six auth email types: magic link, reset password, confirm signup, invite user, change email, and reauthentication.

**Process note — live secret exposed in chat, flagged immediately:** the user pasted a real Google OAuth Client Secret (`GOCSPX-...`) into the conversation while troubleshooting the SMTP fields. Flagged this as compromised-by-exposure on the spot and recommended rotating it via Google Cloud Console → Credentials → Reset Secret, then updating it in Supabase's Google provider settings (not the SMTP fields, which don't use it at all). Not independently confirmed whether the user actually rotated it — worth checking in a future session if this comes up again.

### What was added

- **`supabase/migrations/0045_magic_link_rate_limit.sql`** (new) — a `magic_link_attempts` table, same "RLS enabled, zero client grants, service-role only" shape as `moodle_login_attempts` (0031). Applied to the live project by the user this session (verified live, see below).
- **`src/lib/magic-link-server.ts`** (new) — `checkMagicLinkRateLimit`, a `createServerFn` gating magic-link requests server-side (3 attempts / 15 minutes per target email) *before* the browser is ever allowed to call Supabase's own OTP endpoint — a client-side-only cooldown would be trivially bypassable by calling the Supabase API directly. Closes the one real risk in this feature: someone spamming sign-in-link emails at a specific student's inbox just to annoy them (the link itself is single-use/expiring, so this was never an account-takeover risk, only a nuisance one).
- **`src/lib/magic-link.ts`** (new) — `sendMagicLink()`, the client-side entry point. Calls `signInWithOtp` with `shouldCreateUser: false` — deliberately never a backdoor signup path; only the real `/signup` flow can create an account. Masks "no such account" as a plain success client-side, matching `forgot-password.tsx`'s own documented no-account-enumeration reasoning for `resetPasswordForEmail`.
- **`src/routes/magic-link.tsx`** (new) — mirrors `forgot-password.tsx`'s exact layout/tone. Linked from `login.tsx` as "Email me a sign-in link instead."
- **Also included in this push, per the user's explicit request to review, verify, and ship together** (pre-existing, uncommitted, not authored this session — found already sitting in the working tree with no DEV_LOG entry and no commit history):
  - **`src/routes/forgot-password.tsx` / `reset-password.tsx`** — password reset request + completion pages. Reviewed: `reset-password.tsx` correctly waits for the real `PASSWORD_RECOVERY` auth event (not just assuming a clicked link worked) before allowing a password change, with a 5s timeout that turns an expired/already-used link into a real "This link has expired" state instead of hanging.
  - **`src/lib/modules-api.ts`** — a cache-first rework of `fetchModules`/`fetchModule`: if this device has already cached the catalog (any earlier visit), serve it instantly and refresh in the background, instead of the old network-first behavior that made every dashboard/courses/reader visit wait out a 6s timeout even when a full local cache already existed. Reviewed: only caches `modules`/`materials`, which are publicly-readable catalog data (not per-user), so the device-wide (not per-user-keyed) cache has no privacy implication.
  - **`src/styles.css`** — `--radius` changed from `0.875rem` to `5px`. A real, dramatic visual shift (card corners go from ~22px to ~13px app-wide via the derived `--radius-2xl` scale) — purely cosmetic, flagged to the user before shipping, not something this session evaluated for "is this the right design call," only "does it break anything" (it doesn't).
  - **`src/routes/index.tsx`** — one landing-page carousel slide's copy simplified.

### How it was validated

`tsc --noEmit` and `eslint` clean on every touched/reviewed file; `npm run build` succeeds. Real, UI-independent verification against the live database and live Supabase Auth (avoided fighting an unrelated pre-existing `ResearchSurveyModal` popup that legitimately re-triggers on every fresh `page.goto()` in a Playwright script, which is expected/correct behavior for a real user hard-refreshing repeatedly, not a bug):
- Confirmed `magic_link_attempts` is live (migration 0045 applied): a real `select` against it returns successfully.
- Exercised the exact rate-limit query the server function runs: inserted 3 real rows for a throwaway email, ran the same count-in-window query, confirmed it correctly reports a 4th attempt should be blocked.
- Called `signInWithOtp` directly against a real, existing test account: accepted with no error — real evidence the corrected Gmail SMTP setup actually works end-to-end through Supabase's own send path, not just "the form saved."
- Called `signInWithOtp` against a definitely-nonexistent email: got back Supabase's real `"Signups not allowed for otp"` error, and confirmed the client's own masking regex (`/signup|not allowed/i`) actually matches that real string — proving the no-enumeration masking isn't just theoretical.
- Drove the real app with Playwright (real signed-in test user, created via the Admin API, deleted afterward along with its test rows): dashboard renders on first load, then renders correctly from the cache-first path after a reload with `'Available offline'` visible; a module detail page's title was confirmed identical before and after a reload of that page too.
- Password reset's full link-click-through **could not be verified in this environment**: `generateLink`'s `redirectTo` pointing at `localhost:8081` was silently ignored in favor of the project's configured production Site URL, since local dev isn't in Supabase's redirect allow-list — an environment limitation, not a code issue (same category as other local-vs-production gaps already logged in this file). The request-side API call was confirmed live regardless (Supabase's own platform rate limit kicked in on a second rapid call — real evidence the endpoint is reachable and functioning, not silently broken).

Cleaned up all test artifacts and DB rows afterward: deleted the throwaway test user and its `magic_link_attempts` rows via the Admin API, removed all temporary scripts/screenshots from the working tree.
...
14. ~~Migration `0044_admin_audit_log.sql` needs applying~~ — applied by the user and verified live this session (Feature 86/88): queried the real table and RPC directly (real empty results, not schema errors), then ran a full real end-to-end check — a real super-admin account calling `admin_set_lecturer_role` against a real target account — and confirmed the audit row landed with the exact right `actor_id`/`action`/`target_user_id`/`details`. Test accounts deleted afterward.
