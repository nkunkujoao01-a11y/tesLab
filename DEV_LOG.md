...
14. **New (Feature 86): migration `0044_admin_audit_log.sql` needs applying to the live Supabase project.** Written and verified locally (build/typecheck/lint clean) but never pushed to the live database from this session — no destructive/live-data action was taken. Until it's applied, `/admin/super/audit-log` will show "No admin actions recorded yet" and `recordAdminAction` will log-and-continue rather than actually persist anything (by design — never blocks the ban/delete/lecturer-grant action itself).
