-- Fix: migration 0002's em-dashes ("—", U+2014) were corrupted in transit
-- through the SQL Editor (UTF-8 bytes E2 80 94 got reinterpreted as three
-- separate Latin-1 characters and re-encoded — classic double-encoding
-- mojibake, visible as "â€”" in the stored JSON). Plain ASCII apostrophes
-- survived fine; only the multi-byte character was affected.
--
-- Rather than depend on a SQL-client encoding setting we don't control,
-- this migration re-applies all 14 materials' content using only ASCII
-- punctuation (" - " in place of em-dashes), which is immune to this class
-- of bug entirely.
