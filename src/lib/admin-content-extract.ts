...
/** Reads raw text from a PDF, Markdown, or plain-text file. Markdown/text
 * files are read as-is — a `.md` file already uses the same `#`/`##`/`- `
 * convention pdf-extract.ts produces, so no separate parser is needed; a
 * `.txt` file just has no structure markers, which the fallbacks below
 * handle the same way a heading-less PDF page would. */
